...
def clean(
    ...
    *,
    ...
    allowed_signs: set[str] = set(vocab_filter.get("included_signs", []))
    if not allowed_signs:
        # Fall back to the keys of a {sign_id: count} mapping if the
        # filter wasn't generated by filter_vocabulary.py.
        allowed_signs = set(vocab_filter.keys()) if isinstance(vocab_filter, dict) else set()

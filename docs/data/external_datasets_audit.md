...
annotations were produced by a pretrained keypoint detector would distill that
detector's knowledge into our weights. The constraint perimeter would be honored
on the surface (we never install such a package) while being violated in
substance (our detector would predict whatever that model predicts, minus the
noise floor of distillation loss). ADR 0015 codifies this stance.
...
|---|---|
| **Ultralytics 26K Hand Keypoints** | Labels generated by a pretrained model per Ultralytics' own documentation. Disqualified by ADR 0015. |

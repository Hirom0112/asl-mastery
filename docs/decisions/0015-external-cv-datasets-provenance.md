...
**Date:** 2026-05-21
**Relates to:** [ADR 0011](./0011-landmarks-and-templates-pivot.md) (landmarks + templates pivot), [ADR 0012](./0012-strict-from-scratch-cv-constraint.md) (strict from-scratch CV constraint), [ADR 0008](./0008-public-data-only-training.md) (public-data-only training), [ADR 0009](./0009-asl-citizen-v2.md) (ASL Citizen license analysis), [ADR 0004](./0004-public-sources-only.md) (public sources only for the pilot)
...
3. **Disqualifying label-production mechanisms.** A dataset is rejected if
   ...
   - **MediaPipe** Hands / Pose / Holistic, in any version, with any
     post-processing.
...
4. **License compatibility.** The dataset license must permit research use
   ...
   - Research-only / non-commercial (CC BY-NC, CC BY-NC-ND, MSR-LA-style
     ...
     commercial-cliff review (per ADR 0009 precedent) catches the
     restriction.
...
1. **Substantive equivalence.** Training a network on MediaPipe's predictions
   minimizes a loss against those predictions. The network learns to imitate
   MediaPipe within the noise floor of the distillation loss. Our weights
   become a compressed approximation of MediaPipe's weights. We have not
   built a from-scratch detector; we have built a from-scratch wrapper
   around MediaPipe. The brief's intent — that every learned component be
   the project's own work — is violated even if the literal text is
   satisfied.
2. **Audit credibility.** When a reviewer reads `docs/MODEL.md` and ADR 0012
   and asks "what is from-scratch about this?" the honest answer must be
   "the architecture and the weights." If the labels came from MediaPipe,
   the honest answer is "the architecture, but the weights are a
   distillation of MediaPipe's." That is a different project than the one
   ADR 0012 declares.

By extension, any dataset whose labels were generated by any pretrained CV
model — not just MediaPipe — is disqualified for the same reasons.
...
- Every `data/external/<dataset_name>/` directory contains a
  ...
  in the audit memo.
- A `grep -ri "mediapipe\|openpose" data/external/` returns zero hits in
  any annotation file or provenance record (a hit would indicate either
  a wrongly accepted dataset or a contaminated annotation pipeline).

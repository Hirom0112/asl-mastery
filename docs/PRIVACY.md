...
- The learner app does not contain any code path that sends a video
  ...
  before each release.
- **No third-party vision vendor in the inference path.** The
  recognition models are from-scratch ONNX artifacts served with the
  app; ONNX Runtime Web executes them locally. There is no fetch to a
  third-party model CDN, no third-party WASM runtime to download, and
  no vendor-side telemetry surface.

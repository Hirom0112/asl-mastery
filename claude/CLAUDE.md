...
|---|---|---|
| ~~Recognition path~~ RE-PIVOTED 2026-05-21 | **CURRENT (under [ADR 0011](../docs/decisions/0011-landmarks-and-templates-pivot.md)):** four from-scratch detectors (hand_detector, hand_landmarks, pose, face) → distribution-of-templates sign matcher. Labels for the detectors come from public human-annotated CV datasets per [ADR 0015](../docs/decisions/0015-external-cv-datasets-provenance.md). The single end-to-end CNN of ADRs 0001 / 0010 is superseded — the new pipeline is more sample-efficient at our 13–20 clips/sign data ceiling and produces the interpretable per-Stokoe-parameter decomposition the hint engine needs. **History: ADR 0001 (original Path B) → ADR 0006 (landmark pivot, 2026-05-19) → ADR 0010 (reversal, reinstated Path B, 2026-05-20) → ADR 0011 (re-pivot to from-scratch landmarks + templates, 2026-05-21).** Single source of truth for current state: [`STATUS.md`](../STATUS.md). |
